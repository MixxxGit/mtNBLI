...
//--------------------------------------------------------------------------------------------------
//  hardening of the decoder against damaged / truncated streams
//
//  The original decoder trusts the stream completely : a single flipped bit can make a histogram
//  code write past the end of its table, and a wrong rANS state can make it read past the end of
//  the input buffer.  Both are turned into a plain "damaged" flag (one per thread, because the
//  tiles of a .tnbli file are decoded concurrently), so a broken file is rejected instead of
//  crashing.  See also src/safedecode.h
//--------------------------------------------------------------------------------------------------
inline bool      & nbliRansDamaged () { static __thread bool      f = false; return f; }
inline uint16_t *& nbliRansLimit   () { static __thread uint16_t *p = NULL ; return p; }

inline uint16_t nbliRansRead (uint16_t *&p_buf) {      // read one word, never past the limit
    uint16_t *p_lim = nbliRansLimit ();
    if (p_lim && p_buf >= p_lim) {
        nbliRansDamaged() = true;
        return 0;
    }
    return *(p_buf++);
}
...
public :
    ...
    inline void calculateAccumlate () {
        // the running sum is clamped to NORM_SUM : a damaged (unnormalized) histogram would
        // otherwise make the loop in calculateDecodeLookupTable() write behind the lookup table.
        hist[0].hsum = 0;
        uint32_t sum = 0;
        for (int i=1; i<(1<<VAL_BIT); i++) {
            sum += hist[i-1].hnrm;
            hist[i].hsum = (uint16_t) ((sum > (uint32_t)NORM_SUM) ? (uint32_t)NORM_SUM : sum);
        }
    }
    ...
    inline uint16_t* decode (uint16_t *p_buf) {
        // every write is bounds checked : the run length code (code6) of a damaged stream can
        // otherwise write far behind the table.
        // NOTE : the last code word of a *valid* stream may legally write behind hist[last]
        //        (in the original code those writes land in the following histogram object,
        //        which is harmless because the sums are recomputed afterwards).  One extra
        //        Histogram object is allocated behind the array so that this spill is legal,
        //        anything beyond it means the stream is damaged.
        #define  NBLI_HIST_MAX   (2*(1<<VAL_BIT))
        #define  NBLI_HIST_PUT(v)  do { if (i < NBLI_HIST_MAX) hist[i].hnrm = (uint16_t)(v); else nbliRansDamaged() = true; i++; } while (0)
        for (uint16_t i=0; i<(1<<VAL_BIT); i++) hist[i].hnrm = 0;
        for (uint16_t i=1; i<(1<<VAL_BIT); ) {
            uint16_t code = nbliRansRead (p_buf);
            if (nbliRansDamaged()) break;
            switch (code >> 12) {
                case 0:  case 1:  case 2:  case 3:              // code1
                    NBLI_HIST_PUT (code);
                    break;
                case 4:  case 5:  case 6:  case 7:              // code2
                    NBLI_HIST_PUT ((hist[i-1].hnrm-((code>>7)&0x7F)) & NORM_MASK);
                    NBLI_HIST_PUT ((hist[i-1].hnrm-( code    &0x7F)) & NORM_MASK);
                    break;
                case 8:  case 9:  case 10:  case 11:            // code3
                    NBLI_HIST_PUT ((code>>7) & 0x7F);
                    NBLI_HIST_PUT ((code   ) & 0x7F);
                    break;
                case 12: case 13:                               // code4
                    NBLI_HIST_PUT ((code>>8) & 0x1F);
                    NBLI_HIST_PUT ((code>>4) & 0x0F);
                    NBLI_HIST_PUT ((code   ) & 0x0F);
                    break;
                case 14:                                        // code5
                    NBLI_HIST_PUT ((code>>9) & 0x07);
                    NBLI_HIST_PUT ((code>>6) & 0x07);
                    NBLI_HIST_PUT ((code>>3) & 0x07);
                    NBLI_HIST_PUT ((code   ) & 0x07);
                    break;
                default:                                        // code6
                    uint16_t hrep = (code>>10) & 0x003;
                    int      len  = (code    ) & ((1<<VAL_BIT)-1);
                    for (len++; len>0; len--) NBLI_HIST_PUT (hrep);
                    break;
            }
        }
        #undef NBLI_HIST_PUT
...
private :
    ...
    inline uint16_t decode (uint16_t ctx) {
        if (nbliRansDamaged())          // damaged stream : stop producing symbols
            return 0;
        uint16_t hnrm, hsum;
...
public:
    ...
    inline rANSwithGlobalHistogram (uint16_t *_p_buf, uint16_t *_p_buf_end_for_encoder=NULL) {   // construct
        ...
        hists = new Histogram<VAL_BIT> [N_CTX + 1];   // +1 : spill area, see Histogram::decode()
        ...
        if (IS_ENC) {
            ...
            for (uint16_t ctx=0; ctx<N_CTX; ctx++) {
                ...
                if (nbliRansDamaged())          // damaged stream : leave the rest zeroed
                    return;
